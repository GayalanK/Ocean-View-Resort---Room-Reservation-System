 ...
 * Pure Java implementation - no external libraries required if using Java
 * DB/Derby
 */
public class DatabaseConnection {
    ...
    public String getDatabaseType() {
        if (activeDriver == null)
            return "File-based (No database driver available)";
        if (activeDriver.contains("derby"))
            return "Java DB (Apache Derby)";
        if (activeDriver.contains("h2"))
            return "H2 Database";
        if (activeDriver.contains("sqlite"))
            return "SQLite";
        return "Unknown";
    }
}
